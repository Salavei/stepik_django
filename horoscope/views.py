...
def get_info_about_sign_zodia(request, sign_zodiak:str):
    description = zodiac.get(sign_zodiak, None)
    if sign_zodiak.lower() in zodiac:
        return HttpResponse(f'Знак зодиака - {description}')
    else:
        return HttpResponseNotFound(f'Знак зодиака "{sign_zodiak}" не определен.')
